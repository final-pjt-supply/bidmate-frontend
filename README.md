...
프론트엔드입니다. 공고 수집·자격 판정·추천 계산은 백엔드(`bidmate-backend`)가 맡고,
이 레포는 화면과 백엔드 프록시를 담당합니다.

담당자가 하루 수백 건의 공고에서 면허·지역·실적·인증을 일일이 대조하는 대신, 회사
프로필을 한 번 등록하면 공고별 판정과 그 근거를 먼저 봅니다. 기준은 정보량이 아니라
참여 여부를 얼마나 빨리 판단하느냐입니다.

## 자격 판정

백엔드가 공고마다 10개 축을 평가하고, 화면은 결과를 표시만 합니다. 프론트에서 판정을
다시 계산하거나 추론하지 않습니다.
...
확인필요를 불가와 구분하는 것이 중요합니다. 데이터가 없다는 사실을 미달로 표시하면
참여 가능한 공고를 놓칩니다.
...
미완성 기능은 `src/lib/features.ts`의 환경변수 스위치로 감춥니다. 값이 없으면 꺼지므로,
배포 설정에 넣지 않는 한 노출되지 않습니다. 비드봇은 백엔드 에이전트가 완성될 때까지
기본 비활성입니다.
...
상태관리 라이브러리는 쓰지 않습니다. `useState`로 부족해지면 그때 도입합니다.

## 로컬 개발

Node.js 20 이상, 그리고 함께 띄울 `bidmate-backend`가 필요합니다.
...
표시용 가공은 프론트에서만 합니다. 원본 값은 건드리지 않고 화면에 그릴 때만 바꿉니다 —
업종 코드→한글(`categoryLabel`), 긴 낙찰방법 축약(`shortMethod`), 금액 포맷
(`formatAmount`), D-day 계산(`computeDday`).
...
- 백엔드는 인터넷에 노출되지 않습니다. 브라우저가 Cognito ID 토큰을 같은 오리진
  `/api/*`로 보내면 Next Route Handler가 프라이빗 서브넷으로 중계합니다. 같은 오리진이라
  CORS·혼합콘텐츠 문제가 없고, 백엔드 주소도 클라이언트에 드러나지 않습니다.
- 인스턴스에 직접 접근할 수 없습니다. 보안그룹이 ALB로부터의 80번만 허용합니다.
- SSL은 ALB에서 종단합니다. nginx는 프록시와 이벤트 레이트리밋만 담당합니다.

## 이벤트 수집

행동 로그는 같은 오리진 `/api/events`로 fire-and-forget 전송한 뒤 백엔드로 포워딩합니다.
16종 이벤트 이름은 `EventName` 유니온 타입으로 고정해 오타로 인한 스키마 오염을 막습니다.

| 항목 | 값 | 이유 |
|---|---|---|
| 본문 상한 | 64KB | 파싱 전에 차단해 거대한 본문을 처리하지 않음 |
| 배치 상한 | 요청당 20건 | 한 요청으로 백엔드 버퍼·S3 비용을 밀어붙이는 것 방지 |
| 방문 세션 | 30분 무활동 시 재발급 | 세션 경계 정의 |
| 익명 ID | 영구 보관 | 로그인 전후 여정 연결 |
...
2. `/health`와 홈 화면 렌더를 확인한다 — 프로세스 생존만으로 통과시키지 않는다
3. 통과하면 nginx upstream을 새 슬롯으로 바꾸고 구 슬롯을 정리한다
4. 전환 후 헬스체크가 실패하면 이전 upstream으로 되돌린다

두 인스턴스는 순차로 처리합니다. 동시에 바꾸면 실패했을 때 가용 인스턴스가 0이 됩니다.
...
| 롤백 30초 | 별도 워크플로에 커밋 SHA만 입력, 빌드 생략하고 ECR 기존 이미지 재배포 |
| 런타임 주입 분리 | 백엔드 주소는 컨테이너 실행 시 주입해 이미지 재빌드 없이 교체 |
