...
> **⚠ 최초 측정(2026-07-27) 이후 인프라가 바뀌었다.**
> 당시 구조는 `인터넷 → 단일 EC2 Nginx → 컨테이너`였고, 대상은 그 EC2의 공인 IP였다.
> 2026-08-06부터는 `인터넷 → ALB(2 AZ, TLS 종단) → EC2 2대 → Nginx → 컨테이너`이고
> 그 EC2는 이미 사라졌다. 지금 실행하면 **ALB를 포함한 현재 구조**를 재는 것이므로
> 2026-07-27 결과와 수치를 직접 비교하면 안 된다.
> 당시 결과: [프론트엔드 부하 테스트 결과 (2026-07-27)](https://app.notion.com/p/3aa0fa75d86b8112b523e5791c8e8186)
>
> 또한 부하 발생기를 로컬 PC에서 돌리면 가정용 회선이 병목이 될 수 있다.
> 서버 자체 한계를 재려면 같은 리전(ap-northeast-2)의 EC2에서 실행할 것.
...
```powershell
$env:BASE_URL="https://bidfriend.ai.kr"
$env:PROFILE="smoke"
$env:TARGET="health"
